...
class MentalHealthPage extends StatefulWidget {
  ...
  const MentalHealthPage({super.key, required this.adService});
...
class _MentalHealthPageState extends State<MentalHealthPage> {
  static const _subCategories = [
    ContentType.commonMentalDisorder,
    ContentType.mentalMedication,
  ];

  void _navigate(ContentType type) {
    if (type == ContentType.commonMentalDisorder) {
      _showWarning(
        titleOf: (loc) => loc.selfDiagnosisWarningTitle,
        messageOf: (loc) => loc.selfDiagnosisWarningMessage,
        type: type,
      );
    } else if (type == ContentType.mentalMedication) {
      _showWarning(
  ...
  Widget build(BuildContext context) {
    ...
    final loc = AppLocalizations.of(context)!;
    final gradient = ContentType.mentalHealthHub.gradient;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Row(
          ...
          children: [
            Icon(ContentType.mentalHealthHub.icon, size: 24),
            const SizedBox(width: 8),
            Flexible(
              child: Text(
                ContentType.mentalHealthHub.localizedTitle(loc),
